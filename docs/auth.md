...
- Resume metadata and LinkedIn profile endpoints use the same server-to-server bearer token. The frontend server derives `auth0_sub` from its Auth0 session; the browser cannot supply a user ID or object key.
